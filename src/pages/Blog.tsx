            ...
            <Card className="overflow-hidden">
              <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <CardContent className="p-8">
                  <Badge className="mb-4">{t(`blog.category.${categoryMap[mockBlogPosts[0].category]}`)}</Badge>
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>{mockBlogPosts[0].readTime}{t('general.min')}</span>
                    </div>
                  </div>
                </CardContent>
              </div>
            ...
            {currentPosts.map((post) => (
              <Card key={post.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <CardContent className="p-6">
                  <Badge className="mb-3">{t(`blog.category.${categoryMap[post.category]}`)}</Badge>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {['Guides', 'Jardinage', 'Entretien', 'Sécurité', 'Nouveautés'].map((category) => (
                <Badge key={category} variant="outline" className="px-4 py-2 hover:bg-accent hover:text-white cursor-pointer">
                  {t(`blog.category.${categoryMap[category]}`)}
                </Badge>
              ))}
            </div>
          </div>
